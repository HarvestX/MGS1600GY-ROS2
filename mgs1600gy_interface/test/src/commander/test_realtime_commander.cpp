...
{
protected:
  using PacketHandler = mgs1600gy_interface::PacketHandler;
  using RealtimeCommander = mgs1600gy_interface::RealtimeCommander;
  ...
  virtual void SetUp()
  {
    ...
    this->packet_handler = std::make_shared<PacketHandler>(&mock_port_handler, logger);
    this->commander = std::make_unique<RealtimeCommander>(this->packet_handler, logger, 1ms);
  }
};
...
TEST_F(TestRealtimeCommander, writeAngOK)
{
  ...
  EXPECT_CALL(mock_port_handler, writePort(StrEq(sending), sending.size())).Times(1);
